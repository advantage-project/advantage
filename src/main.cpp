                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (fRejectInsaneFee && nFees > MIN_RELAY_TX_FEE * 1000000000000000) //fee limit in satoshis
            return error("AcceptableInputs: : insane fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 1000000000000000);
                         ...
                         bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        if (fRejectInsaneFee && nFees > txMinFee * 1000000000000000) //fee limit in satoshis
            return error("AcceptableInputs: : insane fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 1000000000000000);
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 16347236 * COIN; // premine
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	if (pindex->nHeight > 1 && pindex->nHeight % 100 == 0) {nReward += 100000; //TreasuryinSats
}
        ...
        if (wallet.CreateCoinStake(wallet, nBits, nSearchInterval, nFees, txCoinStake, key))
        {
            if (txCoinStake.nTime >= pindexBest->GetPastTimeLimit()+1)
            {
                ...
                for (vector<CTransaction>::iterator it = vtx.begin(); it != vtx.end();)
					if (it->nTime > nTime) { it = vtx.erase(it); LogPrintf("Erased a tx entry\n"); }
					else { ++it; }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	...
	ret = 0;
        ...
        ret = blockValue * 25 / 100; // Masternode Reward 25% // Stakers Reward 51% //
    }
    return ret;
}
