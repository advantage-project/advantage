...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
	...
	// Spare the treasury pay amount from staking
	int64_t treasuryPay = blockValue * 0.24;
	if (nHeight > 1) {
		blockValue -= treasuryPay;
	}
	...
	if (nHeight > 1) {
		// Finally pay the treasury
		txNew.vout.resize(payments + 1);
		txNew.vout[payments].scriptPubKey = Params().GetTreasuryRewardScript();
		txNew.vout[payments].nValue = treasuryPay + (nHeight % 100 ? 0 : 100000);
	}
